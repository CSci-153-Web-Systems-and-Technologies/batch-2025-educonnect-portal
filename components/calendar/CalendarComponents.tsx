...
import { SchoolEvent } from "@/hooks/useTeacherCalendar";
  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center text-center h-full">
        ...
        <p className="text-gray-400">No published events for this date.</p>
      </div>
    );
  }
        ...
        <div key={event.id} className="group flex items-start gap-4 p-4 rounded-2xl border border-gray-100 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-all">
          ...
          <div className="flex-1">
            <div className="flex justify-between items-start">
              ...
              <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Published</span>
            </div>
...
export function EventRow({ event, onEdit, onDelete, onPublish, onUnpublish }: EventRowProps) {
  return (
