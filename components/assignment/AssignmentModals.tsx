        ...
        <div className="relative z-10 w-full max-w-lg bg-white dark:bg-neutral-900 rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in-95 border border-gray-200 dark:border-neutral-800">
          ...
          <div className="space-y-6">
             <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-neutral-800/50 border border-gray-200 dark:border-neutral-800"><p className="text-xs text-gray-600 dark:text-gray-400 font-bold uppercase mb-1">Start Date</p><p className="font-bold text-gray-900 dark:text-white">{format(new Date(data.startDate), "MMM d, yyyy")}</p><p className="text-sm text-gray-600 dark:text-gray-400">{format(new Date(data.startDate), "h:mm a")}</p></div>
                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-neutral-800/50 border border-gray-200 dark:border-neutral-800"><p className="text-xs text-gray-600 dark:text-gray-400 font-bold uppercase mb-1">Due Date</p><p className="font-bold text-gray-900 dark:text-white">{format(new Date(data.dueDate), "MMM d, yyyy")}</p><p className="text-sm text-gray-600 dark:text-gray-400">{format(new Date(data.dueDate), "h:mm a")}</p></div>
             </div>
             <div className="space-y-2"><Label className="text-gray-700 dark:text-gray-300">Description</Label><div className="p-4 rounded-2xl bg-gray-50 dark:bg-neutral-800/30 border border-gray-200 dark:border-neutral-800 text-gray-700 dark:text-gray-300 text-sm leading-relaxed min-h-20">{data.description}</div></div>
             <div className="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-neutral-800">
                <div className="flex items-center gap-2"><div className="h-8 w-8 rounded-full bg-gray-100 dark:bg-neutral-800 flex items-center justify-center text-gray-600 dark:text-gray-400"><User className="h-4 w-4" /></div><span className="text-sm text-gray-600 dark:text-gray-500">Created by {data.creatorName || data.creator}</span></div>
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${data.status === 'Published' ? 'bg-green-500/20 text-green-600 dark:text-green-400' : 'bg-amber-500/20 text-amber-600 dark:text-amber-400'}`}>{data.status}</span>
             </div>
          </div>
        </div>
      </div>
...
export function DeleteAssignmentModal({ isOpen, onClose, onConfirm, title }: any) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
        <div className="relative z-10 w-full max-w-md bg-white dark:bg-neutral-900 rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 border border-gray-200 dark:border-neutral-800">
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Delete Assignment?</h2>
            <p className="text-gray-600 dark:text-gray-400 mb-8">Are you sure you want to delete <span className="font-bold text-gray-900 dark:text-white">"{title}"</span>? <br/> This action cannot be undone.</p>
            <div className="grid grid-cols-2 gap-3 w-full"><Button variant="outline" onClick={onClose} className="rounded-xl h-12 border-gray-300 dark:border-neutral-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-neutral-800 hover:text-gray-900 dark:hover:text-white">Cancel</Button><Button onClick={onConfirm} className="rounded-xl h-12 bg-red-600 hover:bg-red-700 text-white">Delete</Button></div>
          </div>
        </div>
      </div>
    );
...
export function PublishAssignmentModal({ isOpen, onClose, onConfirm, title }: PublishModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative z-10 w-full max-w-md bg-white dark:bg-neutral-900 rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 border border-gray-200 dark:border-neutral-800">
        <div className="flex flex-col items-center text-center mb-6">
          <div className="h-14 w-14 bg-blue-500/10 rounded-full flex items-center justify-center mb-4">
             <Globe className="h-7 w-7 text-blue-500" /> 
          </div>
          <h2 className="text-2xl font-serif font-bold text-gray-900 dark:text-white">Publish Assignment</h2>
        </div>
        <p className="text-gray-600 dark:text-gray-400 text-center mb-6 leading-relaxed">
          Are you sure you want to publish <span className="font-semibold text-gray-900 dark:text-white">"{title}"</span>? <br/>
          Students will be able to view this assignment immediately.
        </p>
        <div className="flex gap-4 p-4 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 mb-8">
          <AlertTriangle className="h-5 w-5 shrink-0 mt-0.5" />
          <p className="text-sm font-medium">A notification will be sent to all students.</p>
        </div>
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={onClose} className="rounded-xl px-6 h-11 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-neutral-800">Cancel</Button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-white dark:bg-neutral-900 rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 border border-gray-200 dark:border-neutral-800">
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <h2 className="text-2xl font-serif font-bold text-gray-900 dark:text-white">Unpublish Assignment</h2>
        </div>
        <p className="text-gray-600 dark:text-gray-400 text-center mb-6 leading-relaxed">
          Are you sure you want to unpublish <span className="font-semibold text-gray-900 dark:text-white">"{title}"</span>? <br/>
          This will hide the assignment from the students' view.
        </p>
        <div className="flex gap-4 p-4 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 mb-8">
          <AlertTriangle className="h-5 w-5 shrink-0 mt-0.5" />
